 ...
 *
 * The generic dialect uses exit code 2 for a recognized blocked action.
 * Unknown hosts and malformed payloads may still defer to normal permissions.
 */
...
export function resolveDialect({ argv = [], env = {}, payload = {} } = {}) {
  ...
  if (event === 'PreToolUse') {
    ...
    const isCodex = payload?.turn_id !== undefined;
...
export function normalize(payload, dialect) {
  ...
  // Gemini uses dir_path; directory is retained for older payloads.
  const directory = firstString(toolInput.dir_path, toolInput.directory);
  const pathApi = /^[A-Za-z]:[\\/]/.test(baseCwd) || baseCwd.startsWith('\\\\') ? win32 : posix;
  const cwd = directory ? pathApi.resolve(baseCwd, directory) : baseCwd;
